...
export function T183Print({ id }: { id: string }) {
	...
	/**
	 * A line's value, or `undefined` when the computed return does not carry it.
	 *
	 * ABSENT IS NOT ZERO. This returned `0` for a missing line, and Part 2 asked
	 * for federal line names — `taxableIncome`, `partITaxPayable`,
	 * `partIVTaxPayable` — which an ALBERTA computed return does not emit; it
	 * emits `albertaTaxableIncome`, `albertaTaxPayable` and so on. So an AT1
	 * engagement printed a T183 reading $0 taxable income, $0 Part I, $0 Part IV
	 * and a real $8,000 owing, and an officer was asked to certify it as
	 * "correct, complete, and fully discloses the corporation's income".
	 */
	const foldVal = (line: string): number | undefined => {
		const f = (computed?.fields ?? []).find((x) => x.line === line);
		return f ? Number(f.value) : undefined;
	};
	/** Money, or an em dash — never a fabricated zero for a figure we do not hold. */
	const amount = (v: number | undefined) => (v === undefined ? "—" : money(v));

	const isT2Program = engagement.program === "T2";
	const totalOwing = foldVal("totalOwing");
	const totalTax = totalOwing === undefined ? undefined : totalOwing / 100; // cents → dollars

	/**
	 * The figures Part 2 reports, per program — each computation emits its own
	 * line names and there is no overlap beyond `totalOwing`.
	 */
	const part2Fields = isT2Program
		? [
				{ label: "Taxable income", value: amount(foldVal("taxableIncome")) },
				{
					label: "Part I tax payable",
					value: amount(foldVal("partITaxPayable")),
				},
				{
					label: "Part IV tax payable",
					value: amount(foldVal("partIVTaxPayable")),
				},
				{ label: "Total federal tax owing", value: amount(totalTax) },
			]
		: [
				{
					label: "Alberta taxable income",
					value: amount(foldVal("albertaTaxableIncome")),
				},
				{
					label: "Basic Alberta tax",
					value: amount(foldVal("basicAlbertaTax")),
				},
				{
	...
	const onRecord = async (values: T183Values) => {
		try {
			...
			setRecordOpen(false);
			toast.success(
				"T183 authorization recorded — the return can now be transmitted",
			);
		} catch (e) {
			toast.error(
				e instanceof Error ? e.message : "Could not record the authorization",
			);
		}
	};
		...
		<div>
			<div className="no-print mb-4 flex items-center justify-between gap-3">
				<p className="text-sm text-muted-foreground">
					Print it, have the officer sign it, then record the signature here.
					Filing is blocked until it is recorded, and a recompute invalidates
					it.
				</p>
				<div className="flex items-center gap-2">
					...
					<Button
						...
						disabled={authorizeT183.isPending}
					>
						Record signature
					</Button>
				</div>
				...
				<div style={{ color: "#1f2937" }}>
					...
					>
						<h1 style={{ fontSize: 18, fontWeight: 800, margin: 0 }}>
							T183 CORP: Information Return for Corporations Filing
							Electronically
						</h1>
						<p style={{ fontSize: 12, color: "#555", marginTop: 2 }}>
							Tax year {d(engagement.taxYearStart)} to{" "}
							{d(engagement.taxYearEnd)}
						</p>
					</div>
					...
					<DocumentSection>
						...
						>
							...
							<Field
								...
								value={d(engagement.taxYearStart)}
							/>
					...
					<DocumentSection breakRule="keep-together">
						<h2 style={{ fontSize: 14, fontWeight: 700, margin: "14px 0 8px" }}>
							Part 2: Amounts reported on the return
						</h2>
						<div
							style={{
								display: "grid",
						...
						{part2Fields.some((f) => f.value === "—") && (
							<p style={{ fontSize: 11, color: "#a00", marginTop: 4 }}>
								A dash means the computed return does not carry that figure. Do
								not sign this form until every amount is present — recompute the
								return first.
							</p>
						)}
					</DocumentSection>
					...
					<DocumentSection breakRule="keep-together">
						...
						<p style={{ fontSize: 12, color: "#333", marginBottom: 20 }}>
							I am an authorized signing officer of the corporation. I certify
							that the information given on this form and on the return being
							filed electronically is correct, complete, and fully discloses the
							corporation's income from all sources, and I authorize the
							electronic filer to file this return.
						</p>
						<div
							style={{
								display: "grid",
					...
					<p style={{ fontSize: 10, color: "#777", marginTop: 24 }}>
						The signing officer must sign before the return is transmitted. Keep
						the signed T183 for six years.
					</p>
				</div>
			</DocumentPage>
		</div>
