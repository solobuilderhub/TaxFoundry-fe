...
import {
	OutlineNav,
	type OutlineNode,
} from "@classytic/fluid/client/outline-nav";
import {
	ArrowLeft,
	Calculator,
	FileText,
...
export function ReturnJacket({ id }: { id: string }) {
	...
	const header = (
		<div className="flex flex-wrap items-start justify-between gap-3">
			<div>
				...
				<div className="flex items-center gap-2">
					<h1 className="text-xl font-semibold">
						{engagement.program} jacket · {year}
					</h1>
					<Badge
						variant={STATUS_VARIANT[engagement.status] ?? "outline"}
			...
			<div className="flex items-center gap-2">
				<Link
					href={`/dashboard/engagements/${id}/return`}
				...
				<Link
					href={`/dashboard/engagements/${id}/t183`}
					className={buttonVariants({ variant: "outline" })}
				>
				...
				<Card>
					<CardContent className="flex flex-col items-center gap-3 py-16 text-center">
						...
						<p className="text-sm text-muted-foreground">
							This return hasn’t been computed yet — the jacket appears once you
							compute it.
						</p>
						<Link
							href={`/dashboard/engagements/${id}/return`}
	...
	const doc = buildJacket({
		...
		fields: (computed.fields ?? []) as {
			line: string;
			value: unknown;
			provenance: "engine" | "imported" | "human";
		}[],
		...
		identity: {
			...
			// Line 750 — from the client's registered address, since the return
			// editor has no province control of its own.
	...
	const stats = [
		{
			...
			value:
				fieldVal("taxableIncome") ??
				fieldVal("quebecTaxableIncome") ??
				fieldVal("albertaTaxableIncome"),
		},
		{
			label: "Part I / provincial tax",
			value:
				fieldVal("partITaxPayable") ??
				fieldVal("quebecTaxPayable") ??
				fieldVal("albertaTaxPayable"),
		},
		...
		<div className="space-y-4">
			...
			{engagement.program === "AT1" && (
				<p className="text-xs text-muted-foreground">
					This is a summary of the key figures, not a line-by-line replica of
					the filed AT1 — for the full numbered form (every line, including the
			...
			<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
				{stats.map((s) => (
					<Card
						key={s.label}
						className={cn(s.strong && "border-primary/40 bg-primary/5")}
					>
						<CardContent className="py-4">
							...
							<p
								className={cn(
									"mt-1 tabular-nums",
									s.strong ? "text-2xl font-bold" : "text-xl font-semibold",
								)}
							>
