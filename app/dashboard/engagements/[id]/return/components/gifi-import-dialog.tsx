...
import {
	type CorImportResult,
	type GifiImportResult,
	importCor,
	importGifi,
} from "@/api/gifi";
...
type PreviewRequest =
	| { kind: "gifi"; text: string }
	| { kind: "cor"; content: string };
type PreviewResult = GifiImportResult | CorImportResult;

const isCorResult = (r: PreviewResult): r is CorImportResult =>
	"identification" in r;

/** Business numbers compare on their digits — "123456789 RC0001" and "123456789RC0001" are the same BN. */
const bnDigits = (bn: string | undefined) =>
	(bn ?? "").replace(/\D/g, "").slice(0, 9);
...
export function GifiImportDialog({
	...
	const preview = useMutation<PreviewResult, Error, PreviewRequest>({
		mutationFn: (req) =>
			req.kind === "cor" ? importCor(req.content) : importGifi(req.text),
	});
	const result = preview.data;
	const identification =
		result && isCorResult(result) ? result.identification : undefined;
	const bnMismatch =
		identification?.businessNumber && clientBusinessNumber
			? bnDigits(identification.businessNumber) !==
				bnDigits(clientBusinessNumber)
			: false;
	...
	const rows: { label: string; value: number }[] = result
		? [
				...
				{ label: "Cost of sales", value: result.incomeStatement.costOfSales },
				{
					...
					value: result.incomeStatement.salariesAndWages,
				},
				{ label: "Amortization", value: result.incomeStatement.amortization },
				{
					label: "Other expenses",
					value: result.incomeStatement.otherExpenses,
				},
			]
		: [];
			...
			footer={
				<>
					...
					<Button
						...
						/*
						 * Both reasons, not just one. The mismatch case was explained and
						 * the far commoner one — you have not pressed Preview yet — was
						 * silent, with the Preview control in a different section fifty
						 * lines up the dialog. A disabled button with no reason reads as a
						 * broken one.
						 */
						title={
							bnMismatch
								? "This file is for a different business number than this client"
								: !result
									? "Preview the file first — the figures are applied from what the preview shows"
									: undefined
						}
						onClick={() => {
			...
			<div className="space-y-4">
				<div className="flex flex-wrap items-center gap-2">
					...
					<span className="text-xs text-muted-foreground">
						CSV or plain text, one account per line — or a .cor file.
					</span>
				</div>
					...
					<div className="space-y-2 rounded-lg border p-3 text-sm">
						<div className="flex flex-wrap items-center gap-2">
							<span className="font-medium">
								{identification.corporationName ??
									"Corporation not named in file"}
							</span>
							{identification.businessNumber && (
								<Badge variant="outline">
									BN {identification.businessNumber}
								</Badge>
							)}
							{(identification.taxYearStart || identification.taxYearEnd) && (
								<Badge variant="outline">
									{identification.taxYearStart ?? "?"} →{" "}
									{identification.taxYearEnd ?? "?"}
								</Badge>
							)}
							{identification.provinceCode && (
								<Badge variant="outline">{identification.provinceCode}</Badge>
							)}
						</div>
						{bnMismatch ? (
							<p className="text-destructive">
								This file is for business number {identification.businessNumber}
								, but this engagement's client is {clientBusinessNumber}. Apply
								is disabled — check you have the right corporation's file.
							</p>
						) : (
							clientBusinessNumber &&
							identification.businessNumber && (
								<p className="text-muted-foreground">
									Business number matches this client.
								</p>
							)
						)}
						{result &&
							...
							result.parsingErrors.length > 0 && (
								<p className="text-xs text-destructive">
									{result.parsingErrors.length} line(s) could not be read:{" "}
									{result.parsingErrors.slice(0, 3).join("; ")}
					...
					<div className="space-y-3 rounded-lg border p-3">
						<div className="flex flex-wrap items-center gap-2 text-sm">
							<Badge variant="secondary">
								{result.mappedLines} lines mapped
							</Badge>
							<Badge
								variant={result.totals.balanced ? "default" : "destructive"}
							>
						...
						<div className="grid gap-x-6 gap-y-1 text-sm sm:grid-cols-2">
							...
							<div className="flex justify-between border-b py-1">
								...
								<span className="tabular-nums">
									{money(result.totals.liabilitiesEquity)}
								</span>
							</div>
						</div>
