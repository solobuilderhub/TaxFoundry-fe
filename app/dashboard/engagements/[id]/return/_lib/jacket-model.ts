...
const T2_SPECS: SectionSpec[] = [
	...
	},
	{
		...
		slugs: [
			"donationsClaimed",
			"nonCapitalLossApplied",
			"netCapitalLossApplied",
			"lossCarriedBack",
			"taxableIncome",
		],
	},
	...
	},
	{
		...
		slugs: [
			"nonCapitalLossClosing",
			"netCapitalLossClosing",
			"donationPoolClosing",
			"gripClosing",
			"itcPoolClosing",
			"businessFtcPoolClosing",
			"erdtohClosing",
			"nerdtohClosing",
		],
...
const CO17_SPECS: SectionSpec[] = [
	{
		...
		slugs: [
			...
			"quebecTaxableIncome",
			"quebecSbdIncome",
			"quebecTaxAtSmallBusinessRate",
			"quebecTaxAtGeneralRate",
			"quebecTaxPayable",
			"totalOwing",
		],
	},
...
function refFor(slug: string): string | undefined {
	...
	const m =
		label.match(/\(line (\d+)\)/i) ?? label.match(/\((S\d+[A-Za-z0-9.]*)\)/);
	return m?.[1];
}
...
export function buildJacket(input: BuildInput): JacketDoc {
	...
	const identLines: JacketLine[] = [
		{
			...
			value: ident.corpType ?? identity.corpType ?? "—",
			kind: "text",
		},
		{
			ref: "750",
	...
	if (ident.quebecId)
		identLines.push({
			ref: "NEQ",
			label: "Québec enterprise number",
			value: ident.quebecId,
			kind: "text",
		});
	sections.push({
		id: "identification",
	...
	/*
	 * Settlement — total tax vs what has been paid or credited against it.
	 *
	 * ── The line numbers are the return's own ─────────────────────────────────
	 *
	 * This block printed federal refs (770 / 840 / 590 / 784) on EVERY program,
	 * so an Alberta preparer reconciling the jacket against the AT1 was given
	 * four line numbers that do not appear anywhere on the AT1. Alberta's are
	 * 080 tax payable, 082 instalments and other payments, and 090 balance
	 * unpaid — one signed line rather than a balance/refund pair, which is why
	 * the AT1 side uses a single ref for both outcomes.
	 *
	 * ── And the arithmetic is the filed return's ──────────────────────────────
	 *
	 * AT1 line 090 subtracts the Innovation Employment Grant as well as the
	 * instalments (`albertaBalanceUnpaid` in ca-tax). Netting only instalments
	 * here showed a jacket balance that disagreed with the figure actually
	 * transmitted at 090 for any corporation claiming the grant — the preparer's
	 * summary and the filed return stating two different amounts owing.
	 *
	 * Only the grant is netted because it is the one credit in that formula this
	 * engine computes; 085, 086, 087 and 115 have no computed field yet and are
	 * nil on every return this app produces. When one of them is wired, it
	 * belongs here too.
	 */
	const isAt1 = program === "AT1";
	const totalOwingField = visible.find((f) => f.line === "totalOwing");
	const totalTax = totalOwingField
		? normalize("totalOwing", totalOwingField.value)
		: 0;
	const instalments = toNumber(returnInput.payments?.instalmentsPaid);
	const iegField = visible.find((f) => f.line === "innovationEmploymentGrant");
	const ieg = isAt1 && iegField ? toNumber(iegField.value) : 0;
	const net = totalTax - instalments - ieg;
	const settlement: JacketLine[] = [
		{
			ref: isAt1 ? "080" : "770",
			label: isAt1 ? "Alberta tax payable" : "Total tax payable",
			value: totalTax,
			kind: "money",
			emphasis: "subtotal",
		},
	...
	if (ieg > 0)
		settlement.push({
			ref: "129",
			label: "Innovation Employment Grant",
			value: ieg,
			kind: "money",
			provenance: "engine",
		});
	...
	settlement.push(
		net >= 0
			...
			: {
					// The AT1 has no separate refund line — 090 is signed, and 092 then
					// chooses between a refund and applying it to next year.
					ref: isAt1 ? "090" : "784",
					label: isAt1 ? "Overpayment" : "Refund",
					value: -net,
					kind: "money",
					emphasis: "total",
				},
	...
	sections.push({
		id: "settlement",
		title: "Balance owing or refund",
		lines: settlement,
	});

	// Any unmatched computed line — never drop it silently.
	const leftovers = visible.filter(
		(f) => !used.has(f.line) && f.line !== "totalOwing",
	);
	if (leftovers.length) {
		sections.push({
			id: "other",
			title: "Other computed amounts",
			lines: leftovers.map(fieldToLine),
		});
	}
