		...
		sections: [
			section(
				...
				[
					f.select(
						"province",
						"Province of permanent establishment",
						PROVINCE_OPTIONS,
						{
							required: true,
							description:
								"Drives Schedule 5 provincial tax (AB & QC file separately)",
						},
					),
					f.text("quebecId", "Québec enterprise number (NEQ)", {
						description:
							"CO-17 filing identifier, Revenu Québec / NEQ. Leave blank for a federal-only return.",
					}),
				],
				{
					variant: "card",
			...
			section(
				...
				[
					f.switch(
						...
						"Acquisition of control since year start (s.249(4)) (line 063)",
					),
					f.switch(
						"deemedYearEnd",
						"Deemed tax year-end (s.249(3.1)) (line 066)",
					),
					f.switch(
						"professionalCorp",
						"Professional corporation in a partnership (line 067)",
					),
			...
			section(
				...
				[
					...
					f.switch(
						"addressChanged",
						"Has an address changed since the last return? (lines 010/020/030 — head office, mailing, or books-and-records; this app asks it as one combined question)",
					),
					f.switch(
						"firstReturn",
						"Is this the first year of filing after incorporation? (line 070)",
					),
					f.switch(
						"nonResident",
						"Is the corporation a non-resident? (line 080 asks the OPPOSITE — 'Is the corporation a resident of Canada?')",
					),
					f.switch(
						"amalgamation",
						"Is this the first return after an amalgamation? (line 071)",
					),
					f.switch(
						"windUp",
						"Was there a wind-up of a subsidiary (s.88(1))? (line 072)",
					),
					f.switch(
						"finalReturn",
						"Is this the final return up to dissolution? (line 078)",
					),
				],
				{
					variant: "card",
			...
			section(
				...
				[
					f.switch(
						"foreignAffiliates",
						"Did the corporation own shares in a foreign affiliate? (T1134)",
					),
					f.switch(
						"foreignPropertyOver100k",
						"Foreign property with total cost over CAD $100,000? (T1135)",
					),
					f.switch(
						"nonArmsLengthNonResidentTransactions",
						"Non-arm's-length transactions with non-residents? (T106)",
					),
					f.switch(
						"relatedCorporations",
						"Is the corporation related to any other corporations? (Schedule 9)",
					),
				],
				{
					variant: "card",
					cols: 1,
					description:
						"These trigger separate CRA information returns. Answer them explicitly.",
				},
			),
		],
	}),
