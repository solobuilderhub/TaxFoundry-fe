			...
			section(
				...
				[
					f.array("classes", "CCA classes", [
						field.select("ccaClass", "Class (line 200)", CCA_CLASS_OPTIONS, {
							placeholder: "Select a CCA class",
						}),
						money("openingUCC", "Opening UCC (line 201)", {
						...
						money("claim", "CCA claim", {
							description:
								"Blank = maximum; 0 = claim nothing. A claim above the declining-balance maximum is silently capped at that maximum — this box never files a larger figure than the class actually permits.",
						}),
					]),
				],
				{
			...
			section(
				...
				[
					f.array("class13Layers", "Leasehold layers", [
						...
						field.date(
							"firstRenewalEnd",
							"First renewal termination date (if the lease grants renewal rights)",
						),
						field.switch("isFirstYear", "This is the layer's first tax year"),
						field.switch("aiip", "AIIP property"),
						money(
							"claimedToDate",
							"CCA already claimed on this layer, prior years",
						),
						money("proceeds", "Disposition proceeds attributed to this layer"),
					]),
					money(
						"class13OpeningUCC",
						"Class 13 opening UCC (existing pool + all layers)",
					),
					money("class13Claim", "Class 13 claim", {
						description: "Blank = maximum",
					}),
				],
				{
					variant: "card",
			...
			section(
				...
				[
					f.array("class14Properties", "Limited-life properties", [
						field.text("description", "Description", {
							placeholder: "e.g. 15-year patent",
						}),
						money("capitalCost", "Capital cost"),
						money(
					...
					money(
						"class14OpeningUCC",
						"Class 14 opening UCC (existing pool + all properties)",
					),
					money("class14Claim", "Class 14 claim", {
						description: "Blank = maximum",
					}),
				],
				{
