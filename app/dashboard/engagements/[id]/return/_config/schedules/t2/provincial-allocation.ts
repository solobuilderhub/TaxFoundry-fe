	...
	schema: defineSchema({
		sections: [
			section(
				...
				[
					/*
					 * Column order matches the printed T2 Schedule 5 left to right —
					 * salaries (column B) before gross revenue (column D) — and Form
					 * View's own `COLUMNS` array for this schedule. It did not used to:
					 * this array had revenue before salaries, the reverse of both the
					 * real form and the paper view rendered from the same schedule.
					 * Neither surface binds by POSITION (both write by field name, so
					 * data was never programmatically swapped between them), but a
					 * preparer switching from one view to the other to cross-check a
					 * figure saw the two boxes in a different left-to-right order —
					 * exactly the shape of error that gets a real number typed into
					 * the wrong box by habit (TF_DEV_BUG_LIST_2026-09-18.md, BUG-105).
					 */
					f.array(
						"establishments",
						"Provinces with a permanent establishment",
						[
							field.select(
								"province",
								"Province / territory",
