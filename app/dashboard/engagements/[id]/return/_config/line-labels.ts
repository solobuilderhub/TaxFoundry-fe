...
export const LINE_LABEL: Record<string, string> = {
	...
	//
	// `albertaTaxableIncome` (AT1 line 062) is taxable income on an Alberta
	// basis BEFORE allocation; `amountTaxableInAlberta` (line 066, "062 × 065")
	// is what is actually taxed. Labelled "Alberta taxable income" with no
	// qualifier, 062 read as the single most load-bearing figure on the summary
	// — on a return with a permanent establishment outside Alberta, a preparer
	// at a 60% allocation factor saw the full pre-allocation $200,000 under
	// that caption while only $120,000 was ever taxed in Alberta
	// (TF_DEV_BUG_LIST_2026-09-18.md, BUG-108). 066 was computed the whole
	// time — "the base every rate applies to" — but had never been surfaced as
	// its own field until now (at1-compute.ts).
	albertaTaxableIncome: "Alberta taxable income (before allocation, line 062)",
	amountTaxableInAlberta: "Alberta taxable income (after allocation, line 066)",
