...
import { labelForLine, HIDDEN_LINES } from "../return/_config/line-labels";

const fmtDate = (iso?: string) =>
  iso ? new Date(iso).toLocaleDateString("en-CA") : "—";

const PROV_VARIANT: Record<FieldProvenance, "default" | "secondary" | "outline"> = {
  engine: "default",
  imported: "secondary",
  human: "outline",
};
...
export function EngagementDetail({ id }: { id: string }) {
  ...
  const { latest: computed, isLoading: computedLoading } = useLatestComputedReturn(id);
  ...
  const stageDone = [hasComputed, hasComputed && reviewClear, memoSignedOff, isFiled];
  ...
  const onTransmit = async (values: Record<string, unknown>) => {
    ...
    try {
      await transmit.mutateAsync({ id, certification });
    ...
    <div className="space-y-6">
      <HeaderSection
        ...
        badge={
          isFiled
            ...
            : { text: engagement.status.replace("_", " "), variant: "secondary" }
        }
      />
      ...
      <Card>
        <CardContent className="space-y-4 py-5">
          ...
          <div className="flex flex-col gap-3 rounded-lg border bg-muted/30 p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            {nextStep.cta && (
              <Button onClick={nextStep.onClick} disabled={nextStep.loading} className="shrink-0">
                {nextStep.cta}
              </Button>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push(`/dashboard/engagements/${id}/jacket`)}
            >
          ...
          {gateHint && <p className="text-xs text-muted-foreground">{gateHint}</p>}
          </>
          )}
        </CardContent>
      </Card>
      ...
      <DetailView
        ...
        items={[
          ...
          { label: "Program", value: <Badge variant="secondary">{engagement.program}</Badge> },
          { label: "Tax year", value: period },
          { label: "Status", value: <Badge>{engagement.status.replace("_", " ")}</Badge> },
      ...
      {computed ? (
        <DetailView
          ...
          columns={2}
          items={(computed.fields ?? []).filter((f) => !HIDDEN_LINES.has(f.line)).map((f) => ({
          ...
          items={[
            {
              label: "Status",
              value: (
                <Badge variant={memo.status === "signed_off" ? "default" : "secondary"}>
