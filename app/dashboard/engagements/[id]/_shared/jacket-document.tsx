...
// A tax year is a calendar date, not an instant — see `lib/format-date.ts`.
// Parsing it and rendering in the viewer's timezone printed the WRONG TAX YEAR
// on this document for every preparer west of UTC, Alberta's own included.
const date = formatCalendarDate;

const PROV_STYLE: Record<
	JacketProvenance,
...
function LineRow({ line }: { line: JacketLine }) {
	...
	const rendered =
		line.kind === "money"
			? money(line.value as number)
			: line.kind === "percent"
				? `${((line.value as number) * 100).toFixed(2)}%`
				: String(line.value);
	return (
		<div
			style={{
...
export function JacketDocument({ doc }: { doc: JacketDoc }) {
	...
	return (
		<DocumentPage
			size="A4"
			watermark={h.draft ? "DRAFT" : undefined}
			showPageNumbers
		>
			<div style={{ color: "#1f2937" }}>
				...
				>
					...
					>
						<span>
							<strong>Corporation:</strong> {h.corporation}
						</span>
						<span>
							<strong>Business Number:</strong> {h.businessNumber}
						</span>
						<span>
							<strong>Tax year:</strong> {date(h.taxYearStart)} →{" "}
							{date(h.taxYearEnd)}
						</span>
						<span>
							<strong>Status:</strong> {h.status.replace("_", " ")}
						</span>
					</div>
				</div>
				...
				<DocumentSection breakRule="keep-together">
					<h2 style={{ fontSize: 15, fontWeight: 700, margin: "18px 0 6px" }}>
						Certification
					</h2>
					<p style={{ fontSize: 12, color: "#333", marginBottom: 24 }}>
						I certify that the information given on this return and in any
						documents attached is correct, complete, and fully discloses all the
